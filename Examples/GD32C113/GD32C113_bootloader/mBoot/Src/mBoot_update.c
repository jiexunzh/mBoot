 ...
 ******************************************************************************
 * @file    mBoot_update.c
 * @brief   本文件实现 mBoot 的固件更新流程
 *
 * @details 本模块通过串口接收上位机发送的 BIN 文件并写入 Flash，实现固件升级功能。
 *          使用自定义协议（包含包头、数据、CRC校验）完成数据完整性验证与回传。
 *
 * @note    该文件需要搭配上位机工具，严格按照“BINSIZE + 多包数据”格式进行通讯。
 *
 * @author  ZJX
 * @date    2025-06-20
 * @version 1.0
...
/**
 * @brief  执行一次固件更新流程
 * @note   包括应答上位机、接收文件大小、循环接收固件包、写入 Flash 等操作
 * @retval 正值：BIN 文件总字节数；负值：@ref update_error_t 中定义的错误码
 */
