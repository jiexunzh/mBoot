...
int32_t firmware_update_process(void)
{
    ...
    /* step1: 发送固件更新命令应答“OK\r\n” */
    iap_transmit_string((uint8_t*)NEW_LINE(RESPONSE_OK_CMD));

    /* step2: 等待上位机发来的BIN文件大小字段“BINSIZE=xxxx\r\n”，解析并应答 */
    memset(recv_buf, 0, sizeof(recv_buf));
    if (iap_receive_string(recv_buf, NAK_TIMEOUT) == 0)
	{
		/* “BINSIZE=xxxx\r\n”字段接收超时 */
    ...
    for (int16_t packet_i = 0; packet_i < packet_num; packet_i++)
    {
        ...
        {
            if (iap_receive_byte(recv_buf + i, NAK_TIMEOUT) == 0)
            {
            ...
            && recv_buf[PACKET_SIZE - 1] == crc_h)
        {
            ...
            iap_write_flash(WRITE_START_ADDR, (uint32_t*)(&recv_buf[2]), PACKET_VALID_SIZE / 4);
            iap_transmit_string((uint8_t*)NEW_LINE(RECEIVED_CMD));
            err_num = 0;
        }
        else
        {
            iap_transmit_string((uint8_t*)NEW_LINE(RECEIVED_ERROR_CMD));
            err_num++;
